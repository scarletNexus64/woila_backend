...
class FreemoPayDirect:
    ...
    WITHDRAWAL_PASSWORD = "42oHaGkcySbUqyLfsWjF"  # Utilise SECRET_KEY comme password
    ...
    def _get_basic_auth_header(cls):
        """Génère l'en-tête Basic Auth pour les retraits"""
        credentials = f"{cls.WITHDRAWAL_USERNAME}:{cls.WITHDRAWAL_PASSWORD}"
        encoded_credentials = base64.b64encode(credentials.encode()).decode()
        return f"Basic {encoded_credentials}"
    
    @classmethod
    def init_withdrawal(cls, receiver, amount, external_id, callback):
        """
        Initialise un retrait FreeMo Pay avec Basic Auth.
        
        Args:
            receiver: Numéro de téléphone du bénéficiaire (ex: "237695509408")
            amount: Montant à retirer (string ou number)
            external_id: ID externe pour suivre la transaction
            callback: URL de callback pour les notifications de statut
        
        Returns:
            dict avec le statut et les détails du retrait
        """
        url = f"{cls.BASE_URL}/payment/direct-withdraw"
        ...
        safe_headers = headers.copy()
        safe_headers['Authorization'] = 'Basic [HIDDEN]'
        cls._log_request("POST", url, safe_headers, payload)
        ...
        try:
            ...
            logger.debug(f"[FreeMoPay Withdrawal] Response body: {response.text[:200]}...")
            
            if response.status_code == 200:
                result = response.json()
                result['external_id'] = external_id
                logger.info(f"[FreeMoPay Withdrawal] Retrait initié avec succès - Référence: {result.get('reference')}")
                return result
            else:
                logger.error(f"[FreeMoPay Withdrawal] Erreur de réponse: {response.status_code} - {response.text}")
                return {'status': 'FAILED', 'message': f'Erreur HTTP {response.status_code}', 'external_id': external_id}
                
        except requests.exceptions.Timeout:
            logger.error(f"[FreeMoPay Withdrawal] Timeout lors de l'initialisation du retrait (>{cls.REQUEST_TIMEOUT}s)")
            return {'status': 'FAILED', 'message': 'Timeout de la requête', 'external_id': external_id}
        except requests.exceptions.ConnectionError as e:
            logger.error(f"[FreeMoPay Withdrawal] Erreur de connexion: {str(e)}")
            return {'status': 'FAILED', 'message': f'Erreur de connexion: {str(e)}', 'external_id': external_id}
        except Exception as e:
            logger.error(f"[FreeMoPay Withdrawal] Erreur inattendue: {str(e)}")
            return {'status': 'FAILED', 'message': f'Erreur: {str(e)}', 'external_id': external_id}
    
    @classmethod
    def get_withdrawal_status(cls, reference):
        """
        Obtient le statut d'un retrait par référence.
        Note: Utilise la même méthode que pour les dépôts car l'endpoint de statut est commun.
        """
        return cls.get_payment_status(reference)
    
    @classmethod
    def poll_withdrawal_status(cls, reference, max_duration=None, interval=None):
        """
        Fait du polling sur le statut d'un retrait jusqu'à obtenir SUCCESS/FAILED ou timeout.
        
        Args:
            reference: La référence du retrait
            max_duration: Durée max en secondes (défaut: 120s = 2min)
            interval: Intervalle entre chaque vérification en secondes (défaut: 1s)
        ...
        logger.info(f"[FreeMoPay Withdrawal Polling] Début du polling pour la référence: {reference}")
        logger.info(f"[FreeMoPay Withdrawal Polling] Durée max: {max_duration}s, Intervalle: {interval}s")
        
        while datetime.now() < end_time:
            attempt += 1
            elapsed = (datetime.now() - start_time).total_seconds()
            
            logger.debug(f"[FreeMoPay Withdrawal Polling] Tentative #{attempt} - Temps écoulé: {elapsed:.1f}s")
            
            try:
                # Obtenir le statut actuel
                status_response = cls.get_withdrawal_status(reference)
                current_status = status_response.get('status', 'UNKNOWN')
                reason = status_response.get('reason', '')
                
                logger.info(f"[FreeMoPay Withdrawal Polling] Statut actuel: {current_status} - Raison: {reason}")
                
                # Vérifier si on a un statut final
                if current_status == 'SUCCESS':
                    logger.info(f"[FreeMoPay Withdrawal Polling] ✅ Retrait confirmé après {elapsed:.1f}s")
                    return {
                        'status': 'SUCCESS',
                        'final_status': current_status,
                        'reason': reason,
                    ...
                    logger.warning(f"[FreeMoPay Withdrawal Polling] ❌ Retrait échoué/annulé après {elapsed:.1f}s - Raison: {reason}")
                    return {
                        'status': 'FAILED',
                        'final_status': current_status,
                        'reason': reason,
                    ...
                    # CREATED = transaction créée mais pas encore traitée, traiter comme PENDING
                    # Continuer le polling
                    logger.debug(f"[FreeMoPay Withdrawal Polling] Statut {current_status}, attente de {interval}s...")
                    time.sleep(interval)
                    
                else:
                    # Statut inconnu
                    logger.error(f"[FreeMoPay Withdrawal Polling] Statut inconnu reçu: {current_status}")
                    return {
                        'status': 'FAILED',
                        'final_status': current_status,
                        'reason': f'Statut inconnu: {current_status}',
                ...
                logger.error(f"[FreeMoPay Withdrawal Polling] Erreur lors de la tentative #{attempt}: {str(e)}")
                # On continue le polling malgré l'erreur
                time.sleep(interval)
        
        # Timeout atteint
        elapsed = (datetime.now() - start_time).total_seconds()
        logger.error(f"[FreeMoPay Withdrawal Polling] ⏱️ Timeout atteint après {elapsed:.1f}s et {attempt} tentatives")
        
        # Faire une dernière vérification
        try:
            final_status = cls.get_withdrawal_status(reference)
            return {
                'status': 'TIMEOUT',
                'final_status': final_status.get('status', 'UNKNOWN'),
                'reason': f'Timeout après {max_duration}s',
